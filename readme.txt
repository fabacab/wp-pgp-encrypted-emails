...
* Processes *all* email your site generates.
...
* Visitors can encrypt their comments using an author's public PGP key.
...
Additionally, each of your site's users can supply their own, personal public key and/or X509 for their own email address to have WordPress automatically encrypt any email destined for them. (They merely need to update their user profile pages.) They can choose which type of encryption to use and temporarily deactivate encryption if required. Once set up, all future emails WordPress sends to that user will be encrypted via PGP or S/MIME using their public credentials.
